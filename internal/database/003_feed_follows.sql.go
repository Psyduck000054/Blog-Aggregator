...
const getFFsFromUserID = `-- name: GetFFsFromUserID :many
SELECT id, created_at, updated_at, user_id, feed_id FROM feed_follows WHERE user_id = $1
`

func (q *Queries) GetFFsFromUserID(ctx context.Context, userID uuid.UUID) ([]FeedFollow, error) {
	rows, err := q.db.QueryContext(ctx, getFFsFromUserID, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
